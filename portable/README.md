...
- Предыдущий EXE из исходников `f7a1980` проверен из отдельной папки с PATH только Windows/System32, без PYTHONHOME/PYTHONPATH/VIRTUAL_ENV: health=66, настоящий HTTP smoke, две даты, стабильный повтор, редкая категория, оба пустых исхода и оба проверяемых совета — PASS. Созданное дерево процессов остановлено; порт после проверки закрыт. Эта проверка относится к предыдущему EXE, чей SHA сохранён в build-info.json. Пересборка из `559f7b7` передана интегратору для новой приёмки; её результаты фиксируются отдельно. Проверка каждой версии Windows не заявляется.
